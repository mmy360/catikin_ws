...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      pattern = '<%sd'%length
      buff.write(struct.pack(pattern, *self.res.K))
      _x = self
      buff.write(_get_struct_2i().pack(_x.res.P_row, _x.res.P_col))
      length = len(self.res.P)
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.pack(pattern, *self.res.P))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.res.K = struct.unpack(pattern, str[start:end])
      _x = self
      start = end
      end += 8
      ...
      self.res.P = struct.unpack(pattern, str[start:end])
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e) #most likely buffer underfill
